// Municípios brasileiros (fonte: IBGE, 5571 municípios).
// Gerado por scripts — não editar manualmente. Formato: "Nome|UF" por linha.
// Importado dinamicamente apenas quando o usuário digita em um campo de cidade.

const RAW = `Abadia de Goiás|GO
...
function all(): Municipio[] {
  if (!cache) {
    cache = RAW.split("\n").map((line) => {
      const sep = line.lastIndexOf("|");
      return { nome: line.slice(0, sep), uf: line.slice(sep + 1) };
    });
  }
  return cache;
}
...
/**
 * Busca municípios conforme o usuário digita. Prioriza os que começam com o
 * termo; se uma UF for informada, os dela vêm primeiro.
 */
export function searchMunicipios(query: string, uf?: string, limit = 8): Municipio[] {
  ...
  for (const m of all()) {
    ...
    if (nome.startsWith(q)) starts.push(m);
    else if (nome.includes(q)) contains.push(m);
  }
  const rank = (m: Municipio) => (uf && m.uf === uf ? 0 : 1);
  starts.sort((a, b) => rank(a) - rank(b) || a.nome.localeCompare(b.nome));
  contains.sort((a, b) => rank(a) - rank(b) || a.nome.localeCompare(b.nome));
